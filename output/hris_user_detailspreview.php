...
$page_layouts["hris_user_detailspreview"] = $layout;
...
if($mastertable == "User Manager")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("email") . "=" . make_db_value("email",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	//	mode - 
			...
			$format = $pSet->getViewFormat("mode");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("mode")))
				$class = ' rnr-field-number';
			$row["mode_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("hris_user", "detailspreview"));
