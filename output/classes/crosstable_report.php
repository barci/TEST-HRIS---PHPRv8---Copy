...
class CrossTableReport
{
	...
	{		
		...
		foreach($this->rowinfo as $key_y => $data)
		{
			foreach($data["row_record"]["data"] as $key_x => $fieldData)
			{
				...
				$controlData = array( $field => $fieldData["row_value"] );
				$this->rowinfo[ $key_y ]["row_record"]["data"][$key_x]["row_value"] = $control->showDBValue( $controlData, "" );
			}
			
			if( $data["row_summary"] != "&nbsp;" ) 
			{
				$controlData = array( $field => $data["row_summary"] );
				$this->rowinfo[ $key_y ]["row_summary"] = $control->showDBValue( $controlData, "" );
			}
		}
		
		if( $this->total_summary != "&nbsp;" )
		{
			$controlData = array( $field => $this->total_summary );
			$this->total_summary = $control->showDBValue( $controlData, "" );
		}
		...
		foreach($this->col_summary["data"] as $key => $summaryData)
		{
			...
			$controlData = array( $field => $summaryData["col_summary"] );
			$this->col_summary["data"][$key]["col_summary"] = $control->showDBValue( $controlData, "" );
		}		
	}
	...
	protected function getstrSQL( $strSQL )
	{
		...
		if( $fName != " " )
		{
			...
			{
				$avg_func = ", sum(".$this->connection->addFieldWrappers($fName).") as ".$this->connection->addFieldWrappers("avg_sum")
	...
	public function ajax_refresh_crosstable()
	{
		$reportData = array(
			$this->rowinfo,
			$this->col_summary,
	...
	public function isProjectDB()
	{
		...
		$isDB = false;
		if("hris_affiliation" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_educationprofile" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_job" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_licert" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_personnel" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_tito" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_trainings" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_user" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_personnel" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_personnel" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_journey" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_vacancy" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_job" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_files" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_personnel" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hris_job" == $this->xml_array['tables'][0])
