...
require_once("include/hris_affiliation_variables.php");
...
$page_layouts["hris_affiliation_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	membership - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("membership", $data, $keylink);
			$row["membership_value"] = $value;
			$format = $pSet->getViewFormat("membership");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("membership")))
				$class = ' rnr-field-number';
			$row["membership_class"] = $class;
	//	role - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("hris_affiliation", "detailspreview"));
