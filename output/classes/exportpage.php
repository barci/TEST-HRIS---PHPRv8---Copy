...
class ExportPage extends RunnerPage
{
	...
	public function ExportToCSV($rs, $nPageSize)
	{
		...
		// write data rows
		$iNumberOfRows = 0;
		$this->viewControls->forExport = "csv";
		...
		if( count($totalFieldsData) )
		{
			...
			foreach( $totalFieldsData as $data )
			{
				...
				if( strlen($data["totalsType"]) )
				{
					...
					echo runner_htmlspecialchars( GetTotals($data["fName"], $totals[ $data["fName"] ]["value"], $data["totalsType"], $totals[ $data["fName"] ]["numRows"]
						, $this->pSet->getViewFormat( $data["fName"] ), PAGE_EXPORT) );
				}
				echo "</td>";
			}
			echo "</tr>";
