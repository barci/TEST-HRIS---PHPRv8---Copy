...
$layout = new TLayout("changepwd2", "BreezeOrange", "MobileOrange");
$layout->version = 2;
...
{	
	...
	if($cipherer->isFieldEncrypted($cPasswordField))
		$passvalue = $cipherer->MakeDBValue($cPasswordField, $values["newpass"]);
	else
	{
...
$layout = new TLayout("changepwd_success2", "BreezeOrange", "MobileOrange");
$layout->version = 2;
$layout->blocks["top"] = array();
$layout->containers["changepwd"] = array();
$layout->container_properties["changepwd"] = array(  );
...
foreach($changePwdFields as $fName)
{
	...
	$control[$fName] = array();		
	AssignFunction($control[$fName], "xt_buildeditcontrol", $parameters);
	...
	$xt->assignbyref($fName."_editcontrol", $control[$fName]);
