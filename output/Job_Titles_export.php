...
require_once("include/Job_Titles_variables.php");
...
$page_layouts["Job_Titles_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["department"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["unit"] = refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$keys["jobtitle"] = refine($_REQUEST["mdelete3"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["department"] = urldecode($arr[0]);
			$keys["unit"] = urldecode($arr[1]);
			$keys["jobtitle"] = urldecode($arr[2]);
			$selected_recs[] = $keys;
		}
	}
